...
function Cart() {
     ...
     useEffect(() => {
          const getTotal = () => {
               ...
               }, 0)  // حالت پیشفرض صفر را در نظر بگیرد که خطا ندهد
               setTotal(res)
          }
          getTotal()
     }, [cart]);
     ...
     if(cart.length === 0){
          return <h2 style={{textAlign: "center", fontSize: "3rem"}} > your cart is empty </h2>
     }
  ...
  return (
     ...
     <div className="total">
          <Link to="/">Pay</Link>
          <h3> Total Price: {formatCurrency(total)}</h3>
     </div>
    </>
  )
}
